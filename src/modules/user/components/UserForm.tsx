...
export const UserForm = forwardRef<UserFormRef, UserFormProps>(({
  ...
  return (
    <form className={cn("flex gap-14", className)}>
      <div>
        <InputAvatar
      ...
      <div className="flex-1 grid grid-cols-2 gap-7">
        <div>
          <label htmlFor="fullname" className="block text-primary mb-2">Your Name</label>
          <Input value={formData.fullname} onChangeValue={(fullname) => setFormData({ ...formData, fullname })} name="fullname" />
        </div>
        <div>
          <label htmlFor="username" className="block text-primary mb-2">User Name</label>
        ...
        <div>
          <label htmlFor="birthDate" className="block text-primary mb-2">Date of Birth</label>
          <Input 
            type="date" 
            name="birthDate" 
        ...
        <div>
          <label htmlFor="presentAddress" className="block text-primary mb-2">Present Address</label>
          <Input value={formData.presentAddress} name="presentAddress" onChangeValue={(presentAddress) => setFormData({ ...formData, presentAddress })}/>
        </div>
        <div>
          <label htmlFor="permanentAddress" className="block text-primary mb-2">Permanent Address</label>
        ...
        <div>
          <label htmlFor="city" className="block text-primary mb-2">City</label>
          <Input value={formData.city} name="city" onChangeValue={(city) => setFormData({ ...formData, city })}/>
        </div>
        <div>
          <label htmlFor="postalCode" className="block text-primary mb-2">Postal Code</label>
          <Input value={formData.postalCode} name="postalCode" onChangeValue={(postalCode) => setFormData({ ...formData, postalCode })}/>
        </div>
        <div>
          <label htmlFor="country" className="block text-primary mb-2">Country</label>
